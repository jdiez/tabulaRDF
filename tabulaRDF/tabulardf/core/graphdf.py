...
class FactoryRDFstatements:
    ...
    def __init__(self,
        ...
        self.class_dict = {k: v for k, v in self.config.items() if k not in (prefixes_name, attributes_name)}
    ...
    def _dispatcher(self) -> None:
        ...
        for val in self.attributes.values():
            self._dict[val['colname']] = partial(self._predicate_definition_statement, val['domain'], val['range'])
    ...
    def _get_classuri(self, v) -> str:
        ...
        if self.class_dict['create_uuid']:
            _subject_uri: str = self._resolve_prefix(self.class_dict['subject_uri']) + str(uuid.uuid5(uuid.NAMESPACE_URL, _subject_uri))
        return _subject_uri
    ...
    def _predicate_definition_statement(self,
                                        cv: str,
                                        k: str,
                                        v: str,
                                        create_uuid: bool=False,
                                        literal_prefix: str='http://www.w3.org/2001/XMLSchema') -> None:
        ...
        if _object_uri.startswith(literal_prefix):
            ...
            if create_uuid == True:
                _object_uri: str = _object_uri + str(uuid.uuid5(uuid.NAMESPACE_URL, _object_uri + v))
            else:
    ...
    def _resolve_prefix(self,
                        uri: str,
                        pattern: str = r'^(?P<base>\w+)?(?P<sep>:)?(?P<nspace>[_a-zA-Z0-9#\/]+)?$') -> str:
    ...
    def __call__(self, record: dict[str, str]) -> tuple[str]:
        ...
        for _, value in attrs.items():
            ...
            if value.get('create_uuid'):
                ...
                create_uuid = False
            res_2 = self._predicate_definition_statement(klass_uri, value, record[value['colname']], create_uuid=create_uuid)
            yield res_2
...
class GraphDF:
    ...
    def set_df_uuids(df: pd.DataFrame, id_col: str, index: str, uuid_function: Callable) -> pd.DataFrame:
    ...
    def get_class_dataframe_renamed(self, elements: list[dict[str, dict[str, str]]]) -> Generator[str, dict, pd.DataFrame]:
    ...
    def get_odf(self, class_name: str, class_metatdata: dict[str, Any]) -> pd.DataFrame:
        ...
        Returns:
            pd.DataFrame: [description]
        """
        map_table = {v['name']: v['rename'] for c, v in class_metatdata['attributes'].items()}
...
def get_config_colnames(config: dict[str, Any],
    ...
    class_dict = {k: v for k, v in config.items() if k not in (prefixes_name, attributes_name)}
...
def main(
    table: Annotated[Path, typer.Option(exists=True, dir_okay=False, file_okay=True)] ,
    config: Annotated[Path, typer.Option(exists=True, dir_okay=False, file_okay=True)] ,
    input_format: Annotated[str, typer.Option(help="Table format.")] = "csv",
    output_format: Annotated[str, typer.Option(help="Serialization format.")] = "turtle"):
    """
